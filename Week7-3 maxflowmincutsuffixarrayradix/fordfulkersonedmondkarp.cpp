running time on raw ford fulkers O(Ef) depends on flow
Edmond karp (OVE^2) -even though it looks it doesnt look better than dfs but it is good because it doesnt depend on flow, here we use bfs instead of dfs 
if edge capacities are integer value of max flow is integer
one way to represent it as residual network, where say 9/10 so put 1 in forward edge(10-9 or residual) and 9 in backward edge

number of augmentation<=value of max flow
baseball elimination ,bipartite matching
#include <cstdio>
#include <queue>
#include <cstring>
#include <vector>

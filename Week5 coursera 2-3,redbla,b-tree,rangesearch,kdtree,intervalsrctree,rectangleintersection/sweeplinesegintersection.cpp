//Draw vertical line x=-infinity and start moving line to right .we are interested in relative order of segments where segments will be stored according to y cordinate on sweep line,orderis important because intersecting line segments will have same y cordinate atleast 1 time,event points are those were ordering change 3 types:new segements(left endpoints),end of segments(right endpoints), intersection points,when two segemtns intersect they mmust be adjacent to each other,it is done only for orthogonal line segments
